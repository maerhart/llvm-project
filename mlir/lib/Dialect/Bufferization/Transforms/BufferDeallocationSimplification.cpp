...
struct RemoveDeallocMemrefsContainedInRetained
  ...
  /// The current memref (referred to with 'memrefIdx') must not have a
  /// may-alias relation to any retained memref, and at least one must-alias
  /// relation. If there is no must-aliasing memref in the retain list, we
  /// cannot simply remove the memref as there could be situation in which it
  /// actually has to be deallocated. If it's no-alias, then just proceed, if
  /// it's must-alias we need to update the updated condition returned by the
  /// dealloc operation for that alias.

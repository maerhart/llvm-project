...
Ownership::Ownership(Value indicator) : indicator(indicator), state(State::Unique) {}
...
bool Ownership::isUninitialized() const { return state == State::Uninitialized; }
...
void DeallocationState::updateOwnership(Value memref, Ownership ownership, Block *block) {
...
Ownership DeallocationState::getOwnership(Value memref, Block *block) {
  return ownershipMap[{memref, block}];
}
...
void DeallocationState::getLiveMemrefsIn(Block *block, SmallVectorImpl<Value> &memrefs) {
  SmallVector<Value> liveMemrefs(llvm::make_filter_range(liveness.getLiveIn(block), isMemref));
...
void DeallocationState::getMemrefsToRetain(Block *fromBlock, Block *toBlock,

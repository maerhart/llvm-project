...
private:
  ...
  std::pair<Value, Value> materializeUniqueOwnership(OpBuilder &builder, Value memref, Block *block);
...
std::pair<Value, Value> BufferDeallocation::materializeUniqueOwnership(OpBuilder &builder, Value memref, Block *block) {
  // The interface can only materialize ownership indicators in the same block as the defining op.
  ...
  // If the op implements the interface, query it for a materialized ownership value.
  if (auto deallocOpInterface = dyn_cast<BufferDeallocationOpInterface>(owner))
    return deallocOpInterface.materializeUniqueOwnershipForMemref(state, options, builder, memref);
...
LogicalResult BufferDeallocation::deallocate(Block *block) {
  ...
  for (auto li : liveMemrefs) {
    if (li.getParentRegion() == block->getParent()) {
      state.updateOwnership(li, state.getOwnership(li, li.getParentBlock()), block);
...
Value BufferDeallocation::materializeMemrefWithGuaranteedOwnership(OpBuilder &builder,
                                                           Value memref, Block *block) {
...
BufferDeallocation::handleInterface(BranchOpInterface op) {
  if (op->getNumSuccessors() > 1)
    return op->emitError("BranchOpInterface operations with multiple successors are not supported yet");
  ...
  state.getMemrefsToRetain(block, op->getSuccessor(0), forwardedOperands, toRetain);
...
FailureOr<Operation *> BufferDeallocation::handleInterface(CallOpInterface op) {
  ...
  Operation *funcOp = op.resolveCallable(&state.symbolTable);
  bool isPrivate = true;
  ...
  if (options.privateFuncDynamicOwnership && isPrivate) {
    ...
    for (Value operand : op.getArgOperands()) {
      ...
      auto [memref, condition] = materializeUniqueOwnership(builder, operand, op->getBlock());
...
BufferDeallocation::handleInterface(MemoryEffectOpInterface op) {
  ...
  for (auto operand : llvm::make_filter_range(op->getOperands(), isMemref)) {
    ...
    if (op.getEffectOnValue<MemoryEffects::Free>(operand).has_value())
      state.dropMemrefToDeallocate(operand, block);
  }
  ...
  for (auto res : llvm::make_filter_range(op->getResults(), isMemref)) {
    ...
    if (allocEffect.has_value()) {
      if (isa<SideEffects::AutomaticAllocationScopeResource>(
        ...
        state.updateOwnership(res, getBoolValue(builder, op.getLoc(), false));
        continue;
      }

      state.updateOwnership(res, getBoolValue(builder, op.getLoc(), true));
...
BufferDeallocation::handleInterface(RegionBranchTerminatorOpInterface op) {
  ...
  if (isFunctionWithoutDynamicOwnership(op->getParentOp())) {
    for (OpOperand &val : op->getOpOperands()) {
      ...
      val.set(materializeMemrefWithGuaranteedOwnership(builder, val.get(), op->getBlock()));
    }
  }
...
void BufferDeallocation::populateOwnerships(Operation *op) {
  for (auto res : op->getResults()) {
    ...
    for (auto operand : op->getOperands()) {
      ...
      state.updateOwnership(res, state.getOwnership(operand, operand.getParentBlock()), op->getBlock());
    }
  }
}

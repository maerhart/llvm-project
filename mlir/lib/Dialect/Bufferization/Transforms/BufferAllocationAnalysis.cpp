...
#include "mlir/Dialect/Bufferization/Transforms/BufferAllocationAnalysis.h"
...
AllocationResult AllocationResult::merge(AllocationResult other) const {
  if (kind == other.kind)
    return *this;
...
//===----------------------------------------------------------------------===//
// BufferAllocationAnalysis: alias
//===----------------------------------------------------------------------===//
...
AllocationResult BufferAllocationAnalysis::aliasImpl(Value lhs, Value rhs) {
  if (lhs == rhs)
    return AllocationResult::Same;
  ...
  if (matchPattern(lhs, m_Constant(&lhsAttr))) {
    ...
    if (matchPattern(rhs, m_Constant(&rhsAttr)))
      return AllocationResult::MaybeSame;
    ...
    return succeeded(getAllocEffectFor(rhs, rhsAlloc, rhsAllocScope))
               ? AllocationResult::Different
               : AllocationResult::MaybeSame;
  }
  ...
  if (lhsHasAlloc == rhsHasAlloc) {
    ...
    return lhsHasAlloc ? AllocationResult::Different
                       : AllocationResult::MaybeSame;
  }
  ...
  if (lhsAllocScope) {
    ...
    if (rhsParentOp->isProperAncestor(lhsAllocScope))
      return AllocationResult::Different;
...
/// Recurse on ViewLikeOpInterface starting at 'view' to check if we end up at
/// 'original' at some point. If so, they must at least alias partially.
static bool isViewOf(Value view, Value original, DenseSet<Value> &visited) {
  if (view == original)
    return true;

  if (auto viewOp = view.getDefiningOp<ViewLikeOpInterface>();
      viewOp && !visited.contains(view)) {
    visited.insert(view);
    return isViewOf(viewOp.getViewSource(), original, visited);
  }
...
/// Recurse on ViewLikeOpInterface starting at 'view' to check if we end up at
/// 'original' at some point. If so, they must at least alias partially.
static bool isViewOf(Value view, Value original) {
  DenseSet<Value> visited;
  return isViewOf(view, original, visited);
}

/// Given the two values, return their aliasing behavior.
AllocationResult BufferAllocationAnalysis::alias(Value lhs, Value rhs) {
  if (lhs == rhs)
    return AllocationResult::Same;
  ...
  std::optional<AllocationResult> result;
  for (Value lhsVal : lhsValues) {
    for (Value rhsVal : rhsValues) {
      AllocationResult nextResult = aliasImpl(lhsVal, rhsVal);
      result = result ? result->merge(nextResult) : nextResult;
    }
  }

  if (result->isMaybeSame() && (isViewOf(lhs, rhs) || isViewOf(rhs, lhs)))
    return AllocationResult::Same;

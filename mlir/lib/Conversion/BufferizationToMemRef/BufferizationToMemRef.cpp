...
public:
  ...
  matchAndRewrite(bufferization::DeallocOp op, OpAdaptor adaptor,
    ...
    if (!deallocHelperFunc)
      return op->emitError("library function required for generic lowering, but cannot be automatically inserted when operating on functions");
...
struct BufferizationToMemRefPass
  ...
  void runOnOperation() override {
    if (!isa<ModuleOp, FunctionOpInterface>(getOperation())) {
      emitError(getOperation()->getLoc(), "root operation must be a builtin.module or a function");
      signalPassFailure();
      return;
    }

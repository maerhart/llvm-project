...
static Value getBoolValue(OpBuilder &builder, Location loc, bool value) {
...
public:
  ...
  Ownership getCombined(Ownership other) const {
    ...
    if (matchPattern(indicator, m_Zero()) &&
        matchPattern(other.indicator, m_Zero()))
      return *this;
    if (matchPattern(indicator, m_One()) &&
        matchPattern(other.indicator, m_One()))
      return *this;
...
private:
  ...
  /// implemented by 'op'. If a handler removes the operation, no further
  /// handlers are called on the operation.
  ...
  /// Apply all supported interface handlers to the given op. This is useful
  /// when interface handlers introduce new operations which they have to call
  /// all handlers on themselves.
  ...
  /// inserting auxillary blocks and putting the dealloc op there, however, this
  ...
  /// Make sure that for each forwarded MemRef value, a ownership indicator `i1`
  /// value is forwarded as well such that the successor block knows whether the
  /// MemRef has to be deallocated.
  ///
  /// Example:
  /// ```
  /// ^bb1:
  ...
  template <typename OpTy> OpTy appendOpResults(OpTy op, ArrayRef<Type> types) {
  ...
  void setOwnership(Value owned, Ownership ownership, Block *block = nullptr);
  ...
  /// operations, etc.).
  void populateOwnerships(Operation *op);
  ...
  LogicalResult getMemrefsAndConditionsToDeallocate(
      OpBuilder &builder, Location loc, Block *block,
      SmallVectorImpl<Value> &memrefs, SmallVectorImpl<Value> &conditions);
...
void BufferDeallocation::setOwnership(Value owned, Ownership ownership,
                                      Block *block) {
...
LogicalResult BufferDeallocation::verifyFunctionPreconditions(FunctionOpInterface op) {
...
LogicalResult BufferDeallocation::verifyOperationPreconditions(Operation *op) {
  ...
  if (isa<DeallocOp, memref::DeallocOp>(op))
    return op->emitError(
  ...
  if (op->hasTrait<OpTrait::IsTerminator>()) {
    ...
    // not both.
    if (!isa<BranchOpInterface, RegionBranchTerminatorOpInterface>(
            op) ||
...
BufferDeallocation::updateFunctionSignature(FunctionOpInterface op) {
  SmallVector<TypeRange> returnOperandTypes(llvm::map_range(
      ...
      [](RegionBranchTerminatorOpInterface op) {
        return op.getSuccessorOperands(std::nullopt).getTypes();
      }));
  if (!llvm::all_equal(returnOperandTypes))
    return op->emitError("there are multiple return operations with different operand types");
...
void BufferDeallocation::getMemrefsToRetain(Block *fromBlock, Block *toBlock,
                                            ValueRange destOperands,
                                            SmallVectorImpl<Value> &toRetain) {
...
LogicalResult BufferDeallocation::getMemrefsAndConditionsToDeallocate(
  ...
  for (auto [i, memref] : llvm::enumerate(memrefsToDeallocatePerBlock[block])) {
...
LogicalResult BufferDeallocation::deallocate(Block *block) {
  ...
  for (auto li : liveness.getLiveIn(block)) {
    ...
    if (li.getParentRegion() == block->getParent()) {
      setOwnership(li, ownershipMap[{li, li.getParentBlock()}], block);
      memrefsToDeallocatePerBlock[block].push_back(li);
      continue;
    }

    if (li.getParentRegion()->isProperAncestor(block->getParent())) {
      Value falseVal = getBoolValue(builder, li.getLoc(), false);
      setOwnership(li, falseVal, block);
    }
  }
  ...
  for (unsigned i = 0, e = block->getNumArguments(); i < e; ++i) {
    ...
    // Adhere to function boundary ABI
    if (isFunctionWithoutDynamicOwnership(block->getParentOp()) &&
        block->isEntryBlock()) {
      Value newArg = getBoolValue(builder, arg.getLoc(), false);
      setOwnership(arg, newArg);
      continue;
    }
...
BufferDeallocation::handleInterface(cf::CondBranchOp op) {
  ...
  if (failed(getMemrefsAndConditionsToDeallocate(
          builder, op.getLoc(), op->getBlock(), memrefs, conditions)))
    return failure();
  ...
  auto insertDeallocForBranch =
    ...
    auto deallocOp = builder.create<bufferization::DeallocOp>(
        op.getLoc(), memrefs, adaptedConditions, toRetain);
    ...
    for (auto [retained, ownership] :
      ...
      setOwnership(retained, ownership, op->getBlock());
  ...
  DeallocOp thenTakenDeallocOp = insertDeallocForBranch(
      op.getTrueDest(), op.getTrueDestOperandsMutable(),
      [&](Value cond) {
  ...
  DeallocOp elseTakenDeallocOp = insertDeallocForBranch(
      op.getFalseDest(), op.getFalseDestOperandsMutable(),
      [&](Value cond) {
  ...
  for (Value retained : commonValues) {
    ...
    Value combinedOwnership = builder.create<arith::SelectOp>(
        op.getLoc(), op.getCondition(), thenMapping[retained],
        elseMapping[retained]);
    setOwnership(retained, combinedOwnership, op->getBlock());
  }
...
BufferDeallocation::handleInterface(RegionBranchOpInterface op) {
  ...
  // the structure. For example, getting a mutable operand range of init args is
  // missing or getting the (subset of) results matching the forwarded operands.
  // Additionally, we also make these additional assumptions when we just append
  // the arguments of all basic blocks (except entry blocks of functions) to
  // have an ownership argument for each memref at the end of the arg list. It
  // might be worth adding an interface method for adding additional arguments
  // such that each op can implement it for itself. Otherwise, we have to live
  // with these additional assumptions or build out the interface with a lot of
  // other methods to implement it generically.

  SmallVector<Value> entryOperands(op.getEntrySuccessorOperands(0));
  unsigned numMemrefOperands = llvm::count_if(entryOperands, isMemref);
  Value falseVal = getBoolValue(builder, op.getLoc(), false);
  ...
  for (auto result : llvm::make_filter_range(newOp->getResults(), isMemref)) {
    setOwnership(result, newOp->getResult(counter++));
...
BufferDeallocation::getMemrefWithUniqueOwnership(OpBuilder &builder,
  ...
  Value condition = getBoolValue(builder, memref.getLoc(), true);
  Value newMemref = cloneOp.getResult();
  setOwnership(newMemref, condition);
...
Value BufferDeallocation::getMemrefWithGuaranteedOwnership(OpBuilder &builder,
  ...
  Value maybeClone =
      builder
          ...
          .getResult(0);
  Value trueVal = getBoolValue(builder, memref.getLoc(), true);
  setOwnership(maybeClone, trueVal);
...
BufferDeallocation::handleInterface(BranchOpInterface op) {
  ...
  if (op->getNumSuccessors() == 0)
    return emitError(op.getLoc(), "BranchOpInterface operations with zero "
  ...
  for (auto [retained, ownership] :
       llvm::zip(deallocOp.getRetained(), deallocOp.getUpdatedConditions())) {
    setOwnership(retained, ownership, block);
  }
...
FailureOr<Operation *> BufferDeallocation::handleInterface(CallOpInterface op) {
  ...
  if (privateFuncDynamicOwnership && isPrivate) {
    ...
    for (auto result : llvm::make_filter_range(op->getResults(), isMemref)) {
      setOwnership(result, op->getResult(ownershipCounter++));
  ...
  Value trueVal = getBoolValue(builder, op.getLoc(), true);
  for (auto result : llvm::make_filter_range(op->getResults(), isMemref)) {
    setOwnership(result, trueVal);
    memrefsToDeallocatePerBlock[result.getParentBlock()].push_back(result);
  }
...
BufferDeallocation::handleInterface(MemoryEffectOpInterface op) {
  ...
  for (auto operand : llvm::make_filter_range(op->getOperands(), isMemref)) {
    // TODO: existing deallocation operations are not properly supported yet. In
    // particular, this only works if the originally allocated value is passed
    // to he deallocation op, if a deallocation op is inside a nested region the
    // ownership is also not properly handled because it is always assumed that
    // ownership is not passed to a nested region (this could be improved by
    // doing a pre-pass to collect for which values we have to pass ownership to
    // which region, alternatively, we could pass ownership whenever there is no
    // later use).
    if (op.getEffectOnValue<MemoryEffects::Free>(operand).has_value())
      llvm::erase_if(memrefsToDeallocatePerBlock[block],
                     [&](const auto &memref) { return memref == operand; });
  }
  ...
  for (auto res : llvm::make_filter_range(op->getResults(), isMemref)) {
    ...
    if (allocEffect.has_value()) {
      if (isa<SideEffects::AutomaticAllocationScopeResource>(
        ...
        // typed operand, e.g., something like `clone` but for auto-managed
        // allocations, such that the default ownership population does not
        // assume aliasing with those MemRef operands.
        clearOwnershipOf(res, block);
        setOwnership(res, getBoolValue(builder, op.getLoc(), false));
        continue;
      }

      setOwnership(res, getBoolValue(builder, op.getLoc(), true));
...
BufferDeallocation::handleInterface(RegionBranchTerminatorOpInterface op) {
  ...
  if (isFunctionWithoutDynamicOwnership(op->getParentOp())) {
  ...
  // about, but we would need to check now many successors there are and under
  // wich condition they are taken, etc.

  MutableOperandRange operands = op.getMutableSuccessorOperands(std::nullopt);
...
void BufferDeallocation::populateOwnerships(Operation *op) {

...
BufferDeallocation::handleInterface(BranchOpInterface op) {
  ...
  if (op->getNumSuccessors() != 1)
    return emitError(op.getLoc(), "only BranchOpInterface operations with exactly "
...
BufferDeallocation::handleInterface(MemoryEffectOpInterface op) {
  ...
  for (auto operand : llvm::make_filter_range(op->getOperands(), isMemref)) {
    ...
    // to the deallocation op, if a deallocation op is inside a nested region
    // the ownership is also not properly handled because it is always assumed
    // that ownership is not passed to a nested region (this could be improved
    // by doing a pre-pass to collect for which values we have to pass ownership
    // to which region, alternatively, we could pass ownership whenever there is
    // no later use).
